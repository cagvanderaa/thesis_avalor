...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        ...
        cam_coords = np.array([current_pose_uav1.position.x, current_pose_uav1.position.y, current_pose_uav1.position.z])

        orientation_q = current_pose_uav1.orientation
        orientation_list = [orientation_q.x, orientation_q.y, orientation_q.z, orientation_q.w]

        roll, pitch, yaw = tf.transformations.euler_from_quaternion(orientation_list)
        angles = (yaw, pitch, roll)

        P = np.array([current_pose_uav2.position.x, current_pose_uav2.position.y, current_pose_uav2.position.z])
